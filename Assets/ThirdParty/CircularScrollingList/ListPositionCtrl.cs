...
public class ListPositionCtrl : MonoBehaviour, IControlEventHandler
{
    ...
    public event Action<ListBoxBase> onCenteredBoxChanged;
    public event Action<ListBoxBase> onCenteredBoxExit;
    /*===============================*/

    // The canvas plane which the scrolling list is at.
    public Canvas preferedCanvas;
    ...
    public ListBoxBase currentCenteredBox { get; private set; }
    private ListBoxBase centeredBoxTemp;
    ...
    public void Initialize()
    {
        ...
        foreach (ListBoxBase listBox in listBoxes)
        {
            listBox.Initialize(this);
            ((ListItem)listBox).m_Highlight.enabled = false;
        }
        
        centeredBoxTemp = GetCenteredBox();
        currentCenteredBox = centeredBoxTemp;
        ((ListItem)currentCenteredBox).m_Highlight.enabled = true;
    }

    private void InitializePositionVars()
    {
        /* The the reference of canvas plane */
        if (preferedCanvas != null)
        {
            _parentCanvas = preferedCanvas;
        }
        else
        {
    ...
    private void Update()
    {
        //GetCenteredBox().ShowBoxInfo();
        if (_movementCtrl != null && !_movementCtrl.IsMovementEnded())
        {
            ...
            if (centeredBoxTemp != currentCenteredBox)
            {
                // Centered box exit event.
                onCenteredBoxExit?.Invoke(currentCenteredBox);

                // Centered box change event.
                currentCenteredBox = centeredBoxTemp;
                onCenteredBoxChanged?.Invoke(centeredBoxTemp);
            }
        }
    }
        ...
        switch (direction)
        {
            case Direction.Vertical:
                foreach (ListBoxBase listBox in listBoxes)
                {
                    ...
                    if (listBox == null || !listBox.isActiveAndEnabled)
                        continue;
    ...
    {
        int currCenterId = GetCenteredContentID();
        int length = listBank.GetListLength();

        int diff = index - currCenterId;
        int diff2 = diff > 0 ? diff - length : diff + length;
        if (Mathf.Abs(diff) < Mathf.Abs(diff2))
        {
            SetUnitMove(diff);
        }
        else
	 ...
	 * to the center.
	 */
    public ListBoxBase GetCenteredBox()
    {
        float minPosition = Mathf.Infinity;
        float position;
        ListBoxBase candidateBox = null;
