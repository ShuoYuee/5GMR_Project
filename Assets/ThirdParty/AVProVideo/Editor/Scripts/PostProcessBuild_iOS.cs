...
namespace RenderHeads.Media.AVProVideo.Editor
{
	public class PostProcessBuild_iOS
	{
		...
		{
			#if UNITY_EDITOR_OSX
				string cmd = string.Format("if [ ! -x \"{0}\" ]; then echo \"Fixing plugin binary execute permissions\"; chmod a+x \"{0}\"; else echo \"Plugin binary has execute permissions\"; fi", path);
		...
		public static void ModifyProject(BuildTarget target, string path)
		{
			...
			if (pluginPath.Length > 0)
			{
				...
				SetFileExecutePermission(pluginPath + "/AVProVideo");
